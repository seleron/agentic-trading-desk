# Address Claude review on PR #2 (rounds 1–0)
...
- 7/8
- Guard the backtest step's runtime `import yfinance` so the orchestrator degrades safely when yfinance is absent (the MTF block falls back to daily
- but the backtest import is unguarded and will raise).
